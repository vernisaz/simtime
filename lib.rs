...
pub fn seconds_from_epoch(
    ...
    if day < 1
        || day
            > MON_DAYS[(month as usize) - 1]
                + if month == 2 && year_len(year) == 366 {
                    1
                } else {
                    0
                }
    ...
    if hour > 23 {
        return Err("the hour out of the day length");
    }
    seconds += (hour as u64) * 60 * 60;
    if minute > 59 {
        return Err("the minute out of the hour length");
    }
    seconds += (minute as u64) * 60;
    if second > 59 {
        return Err("the second out of the minute length");
    }
